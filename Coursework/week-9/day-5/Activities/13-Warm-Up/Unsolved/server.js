...
app.get("/markread/:id", function (req, res) {
  ...
  db.books.update({ _id: mongojs.ObjectID(req.params.id) }, { $set: { read: true } }, function (error, edited) {
    console.log(edited);
    res.send(edited);
  })
...
app.get("/markunread/:id", function (req, res) {
  // Remember: when searching by an id, the id needs to be passed in
  // as (mongojs.ObjectId(IdYouWantToFind))
  db.books.update({ _id: mongojs.ObjectID(req.params.id) }, { $set: { read: false } }, function (error, edited) {
    console.log(edited);
    res.send(edited);
  })
});
